#!/usr/bin/env bash
# Apps CachyOS/Hyprland: capturar instalados → meus-apps.txt | instalar a partir do arquivo
# Uso: bash cachyos-hyprland/scripts/apps.sh capturar | instalar
...
capturar() {
  ...
  AUR="$(aur_helper)"
  {
    ...
    echo "# Edite e rode: bash cachyos-hyprland/scripts/apps.sh instalar"
    echo ""
    echo "=== PACMAN ==="
...
instalar() {
  [ ! -f "$APPS_FILE" ] && echo "Arquivo $APPS_FILE não encontrado. Rode: bash cachyos-hyprland/scripts/apps.sh capturar" && exit 1
  ...
  if [ -n "$pacman_pkgs" ]; then
    echo ">>> Pacman..."
    # Tenta pacman primeiro; o que falhar (AUR), passa pro helper
    if [ -n "$AUR" ]; then
